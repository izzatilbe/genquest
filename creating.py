...
def genQuestion(line):
    ...
    if all(key in  bucket for key in l1): #'NNP', 'VBG', 'VBZ', 'IN' in sentence.
        question = 'Imagine another way in' + ' '+ line.words[bucket['VBG']] + ' ' + line.words[bucket['NNP']] + '.'
        output.append(question)

    elif all(key in  bucket for key in l2): #'NNP', 'VBZ' in sentence.
        question = 'Imagine another way in' + ' '+ line.words[bucket['VBG']] + ' ' + line.words[bucket['NNP']] + '.'
        output.append(question)

    elif all(key in  bucket for key in l3): #'PRP', 'VBG', 'VBZ', 'IN' in sentence.
        question = 'What can you propose to strengthen in' + ' ' + line.words[bucket['VBG']] +' '+ line.words[bucket['PRP']]+ '?'
        output.append(question)
        ...
        question = 'What would happen if' + ' '+ line.words[bucket['NN']] +' did not exist?' 
        output.append(question)

    elif all(key in bucket for key in l8): #'NNP', 'VBZ', 'JJ' in sentence.
        question = 'Design new ways to improve' + ' ' + line.words[bucket['NNP']] + '.' 
        output.append(question)

    elif all(key in  bucket for key in l9): #'NNP', 'VBZ', 'NN' in sentence.
        question = 'Can you compose new and unusual uses for' + ' ' + line.words[bucket['NNP']] + '.'  
        output.append(question)
